...
public class CachingEmailExceptionServiceImpl extends EmailExceptionServiceImpl {
	...
	protected void sendEmail(
		...
		if (cache.get(cacheKey) == null) {
			...
			//02/02/2010
			//TEST SI ON A L'ERREUR CharChunk de uPortal ou l'erreur FaceletViewHandler.renderView(FaceletViewHandler.java:543) :
			boolean erreurUportal = false;
			Throwable cause = t;
			while(!erreurUportal && cause!=null){
				StackTraceElement[] st = cause.getStackTrace();
				for(StackTraceElement s : st){
					if(s.getMethodName().equals("toStringInternal")
							&& s.getFileName().equals("CharChunk.java")){
						erreurUportal=true;
					}
					if(s.getMethodName().equals("append")
							&& s.getFileName().equals("CharChunk.java")){
						erreurUportal=true;
					}
					if(s.getMethodName().equals("renderView")
							&& s.getFileName().equals("FaceletViewHandler.java")){
						erreurUportal=true;
					}
			...
			//02/02/2010
			//On envoit pas de mail si on a l'erreur CharChunk de uPortal ou l'erreur FaceletViewHandler.renderView(FaceletViewHandler.java:543) :
			if(!erreurUportal)
