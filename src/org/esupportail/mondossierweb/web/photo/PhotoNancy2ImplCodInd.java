 ...
 */
public class PhotoNancy2ImplCodInd implements IPhoto {
	/**
	 * Un logger.
	 */
	private static final Logger LOG = new LoggerImpl(PhotoNancy2ImplCodInd.class);
	/**
	 ...
	 * constructeur vide.
	 */
	public PhotoNancy2ImplCodInd() {
		super();
		forserver = false;
	}
	public String toString(){
		return "Bean PhotoNancy2CodInd : tc="+tc.getCode();
	}
	...
	public String getUrlPhoto(final String cod_ind, String cod_etu) {
		...
		if (tc != null && tc.isValid(DELAI_TICKET_SEC)) {
			...
			try {
				url = photoClient.computeURLforCode(Category.ETUDIANT, cod_ind, tc);
	 ...
	 */
	public void checkTicket(final String cod_ind) {
	 ...
	 * @param cod_ind
	 */
	public void init(final String cod_ind, String loginUser) {
		...
		if (forserver || tc == null || (tc != null && !tc.isValid(DELAI_TICKET_SEC))) {
			try {
				tc = photoClient.getTicket(PhotoClient.MODE_NORMAL,
						remoteadress, "ID", cod_ind, loginUser);
